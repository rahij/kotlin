/*
 * Copyright 2010-2020 JetBrains s.r.o. and Kotlin Programming Language contributors.
 * Use of this source code is governed by the Apache 2.0 license that can be found in the license/LICENSE.txt file.
 */

package org.jetbrains.kotlin.fir.resolve.transformers

import org.jetbrains.kotlin.fir.declarations.FirFile
import org.jetbrains.kotlin.fir.resolve.ScopeSession
import org.jetbrains.kotlin.fir.visitors.FirTransformer

@RequiresOptIn(message = "Should be used just only in resolve processor")
annotation class AdapterForResolveProcessor

abstract class FirResolveProcessor(val state: State) {
    abstract fun processFiles(files: List<FirFile>)

    class State(val scopeSession: ScopeSession)
}

abstract class FirTransformerBasedResolveProcessor(state: State) : FirResolveProcessor(state) {
    abstract val transformer: FirTransformer<Nothing?>

    override fun processFiles(files: List<FirFile>) {
        for (it in files) {
            it.transform<FirFile, Nothing?>(transformer, null)
        }
    }
}